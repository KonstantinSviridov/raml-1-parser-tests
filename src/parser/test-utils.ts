...
import RamlWrapper08 = parser.api08;
...
export function loadApi(name: string, neverExpand = false):hl.IHighLevelNode {
  var api = parser.parseSync(name);
...
export function loadRAML(ramlPath : string) : hl.BasicNode {
  var opt = parser.loadRAMLSync(ramlPath,[]);
  return opt;
}
...
export function expandHighIfNeeded(original : hl.IHighLevelNode) : hl.IHighLevelNode {
  ...
  return ((<any>wrapper).expand((<any>global).isLibExpanded)).highLevel();
}
